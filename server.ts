...
function routeToModule(pathname: string): string {
  ...
  if (pathname.startsWith("/api/inventory") || pathname.startsWith("/api/warehouses")) return "inventory";
...
function hasServerPermission(user: ApiUser, moduleName: string, action: string) {
  ...
  return permissionAliases(moduleName).some((key) => {
    ...
    if (action === "read") {
      return directModuleAllowed || permissions[`${key}.read`] === true || nested?.read === true || nested?.view === true;
    }
    if (action === "delete") {
      return permissions[`${key}.delete`] === true || nested?.delete === true || (directModuleAllowed && legacyDeleteAllowed);
    }
    if (action === "update") {
      return permissions[`${key}.update`] === true || permissions[`${key}.edit`] === true || nested?.update === true || nested?.edit === true || (directModuleAllowed && legacyEditAllowed);
    }
    if (action === "approve") {
      return permissions[`${key}.approve`] === true || nested?.approve === true || (directModuleAllowed && legacyEditAllowed);
    }
    if (action === "print" || action === "export") {
      return permissions[`${key}.${action}`] === true || nested?.[action] === true || (directModuleAllowed && legacyEditAllowed);
    }
    return permissions[`${key}.create`] === true || permissions[`${key}.add`] === true || nested?.create === true || nested?.add === true || (directModuleAllowed && legacyEditAllowed);
  });
}
...
async function startServer() {
  ...
  app.use(hrRouter);
  // app.use(warehousesRouter); // old - replaced above
