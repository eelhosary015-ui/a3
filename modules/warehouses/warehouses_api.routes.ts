  ...
  if (existing.rows.length > 0) {
    ...
    await client.query(
      `INSERT INTO inventory_items (warehouse_id, ingredient_id, quantity, reserved, in_transit, available)
       VALUES ($1,$2,$3,$4,$5,GREATEST($3 - $4, 0))`,
      [
        opts.warehouse_id,
        opts.ingredient_id,
  ...
  try {
    ...
    let query = `
      SELECT w.*,
        ...
        (SELECT COALESCE(SUM(i.quantity * COALESCE(ing.avg_cost,0)),0)
         FROM inventory_items i
         LEFT JOIN ingredients ing ON ing.id = i.ingredient_id
         WHERE i.warehouse_id = w.id) AS stock_value
      FROM warehouses w
      WHERE 1=1
    `;
    const params: any[] = [];
    if (moduleFilter && moduleFilter !== "all") {
      ...
      query += ` AND (w.linked_module = $${params.length} OR w.linked_module = 'all' OR w.linked_module = 'general' OR w.linked_modules LIKE '%${moduleFilter}%')`;
    }
    query += ` ORDER BY w.is_default DESC, w.is_module_default DESC, w.id ASC`;
    ...
    let query = `
      SELECT t.*, w.name AS warehouse_name, w.code AS warehouse_code,
        ...
        (SELECT COALESCE(SUM((item->>'quantity')::numeric * COALESCE((item->>'price')::numeric,0)),0)
         FROM json_array_elements(t.items::json) AS item) AS total_value
...
router.get("/api/inventory/counts/prepare-new", async (req, res) => {
  try {
    ...
    const formattedItems = itemsRes.rows.map((row: any) => ({
      ...
      code: row.code || `ITM-${row.product_id}`,
      name: row.name,
      unit: row.unit,
      warehouse_id: row.warehouse_id,
      warehouse_name: row.warehouse_name,
      book_quantity: Number(row.book_quantity || 0),
      physical_quantity: Number(row.book_quantity || 0), // Defaults to book qty for seamless counting
