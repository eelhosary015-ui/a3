...
import {
  ...
  ArrowDownRight, Check, Bookmark, Database
} from "lucide-react";
...
export const EnterpriseStockBalanceView: React.FC<EnterpriseStockBalanceViewProps> = ({
  ...
  const categories = useMemo(() => {
    ...
    ingredients.forEach(i => { if (i.category) set.add(i.category); });
  ...
  const exportToExcelComprehensive = () => {
    try {
      const exportData = sortedItems.map(i => ({
        ...
        "كود الصنف": i.ingredient_code || i.code,
        "الباركود": i.barcode || "",
        "التصنيف": i.category || "",
        "الماركة": i.brand || "",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        {/* Card 5: Overstock */}
        <div
          onClick={() => { setSelectedStockStatus("overstock"); }}
          className={`rounded-2xl p-4 border shadow-xs flex items-center justify-between cursor-pointer transition-colors ${
        ...
        <div
          onClick={() => { setSelectedStockStatus("fully_reserved"); }}
          className={`rounded-2xl p-4 border shadow-xs flex items-center justify-between cursor-pointer transition-colors ${
        ...
        {/* Card 7: In Transit */}
        <div
          onClick={() => { setSelectedStockStatus("in_transit"); }}
          className={`rounded-2xl p-4 border shadow-xs flex items-center justify-between cursor-pointer transition-colors ${
        ...
        <div
          onClick={() => { setFilterHasBatch(true); }}
          className="bg-white hover:bg-amber-50/50 rounded-2xl p-4 border border-slate-200 shadow-xs flex items-center justify-between cursor-pointer transition-colors"
        >
          <div>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2 mt-0.5 text-[11px] text-slate-500">
                                {item.ingredient_code && <span className="font-mono text-orange-600 font-bold">{item.ingredient_code}</span>}
