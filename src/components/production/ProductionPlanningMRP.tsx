...
const MOCK_MRP: MRPRequest[] = [
  { id: '1', productName: 'لوح خشب زان 2م', requiredQty: 5000, availableQty: 2000, deficit: 3000, dueDate: '2026-06-25', status: 'planned' },
  { id: '2', productName: 'مسامير صلب 5سم', requiredQty: 20000, availableQty: 5000, deficit: 15000, dueDate: '2026-06-20', status: 'ordered' },
];
...
export function ProductionPlanningMRP() {
  ...
  const [forecastHistSales, setForecastHistSales] = useState(2400); 
  const [forecastGrowth, setForecastGrowth] = useState(12); // percentage
  const [forecastSeason, setForecastSeason] = useState(1.3); // High peak factor
  const [forecastProductCode, setForecastProductCode] = useState('FIN-500');
  ...
  const [productsList] = useState<any[]>(() => {
    ...
    return [
      { id: '1', code: 'FIN-500', name: 'سيارة سيدان', type: 'finished' },
      { id: '2', code: 'SFP-102', name: 'إطار سيارة (نصف مصنع)', type: 'semi_finished' }
    ];
  });
  ...
  const [workCenters, setWorkCenters] = useState<any[]>(() => {
    ...
    return [
      { id: '1', code: 'MC-101', name: 'غرفة القطع بالليزر', capacity: 160, currentAllocatedLoad: 120, unit: 'ساعة' },
      { id: '2', code: 'LN-001', name: 'خط التجميع الرئيسي', capacity: 120, currentAllocatedLoad: 145, unit: 'ساعة' } // Overload!
    ];
  });
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-2">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <div className="space-y-4 text-right">
            {workCenters.map(wc => {
              const cap = wc.capacity || 160;
              const load = wc.currentAllocatedLoad || 120;
              const ratio = Math.round((load / cap) * 100);
                  ...
                  <div className="flex justify-between items-center text-[10px] mt-1">
                    {isOverloaded ? (
                      <span className="text-rose-600 font-bold flex items-center gap-0.5">
                        ...
                        عنق زجاجة! تحميل زائد ({ratio - 100}%). اقترح عمل إضافي أو فرز المكونات.
                      </span>
                    ) : (
