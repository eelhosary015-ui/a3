...
const SAMPLE_PRODUCTS: ProductDef[] = [
  { id: 'p1', code: 'PROD-A72', name: 'قالب ألومنيوم مطلي تيفلون', category: 'أواني المطبخ', type: 'finished', unit: 'قطعة', costMethod: 'standard' },
  { id: 'p2', code: 'PROD-B14', name: 'غطاء زجاجي بقطر ٢٨ سم', category: 'أغطية الزجاج', type: 'finished', unit: 'قطعة', costMethod: 'standard' },
  { id: 'p3', code: 'PROD-C50', name: 'مقبض حراري عازل للحرارة', category: 'اكسسوارات', type: 'semi_finished', unit: 'قطعة', costMethod: 'standard' },
];

const SAMPLE_WORK_CENTERS: WorkCenter[] = [
  { id: 'wc1', code: 'WC-M1', name: 'مركز تشكيل وقص الصاج', type: 'machine', capacity: 100, costPerHour: 180, efficiency: 0.95, laborCost: 75, machineCost: 65, overheadCost: 40 },
  { id: 'wc2', code: 'WC-L1', name: 'خط طلاء وحقن التيفلون التلقائي', type: 'line', capacity: 200, costPerHour: 320, efficiency: 0.90, laborCost: 120, machineCost: 140, overheadCost: 60 },
  { id: 'wc3', code: 'WC-A1', name: 'مركز التجميع والتعبئة اليدوية', type: 'labor', capacity: 80, costPerHour: 120, efficiency: 0.98, laborCost: 90, machineCost: 15, overheadCost: 15 },
];

const SAMPLE_ORDERS: ProductionOrder[] = [
  { id: 'ord1', orderNumber: 'PRD-2026-001', productId: 'p1', quantity: 1500, bomId: 'bom1', startDate: '2026-06-10', endDate: '2026-06-15', priority: 'high', status: 'completed', progress: 100 },
  { id: 'ord2', orderNumber: 'PRD-2026-002', productId: 'p2', quantity: 2400, bomId: 'bom2', startDate: '2026-06-15', endDate: '2026-06-20', priority: 'normal', status: 'in_progress', progress: 65 },
  { id: 'ord3', orderNumber: 'PRD-2026-003', productId: 'p3', quantity: 5000, bomId: 'bom3', startDate: '2026-06-18', endDate: '2026-06-28', priority: 'low', status: 'planned', progress: 0 },
];

const SAMPLE_SHOPFLOOR = [
  { id: 'job1', orderNumber: 'PRD-2026-001', producedQty: 1480, scrapQty: 45, status: 'completed' },
  { id: 'job2', orderNumber: 'PRD-2026-002', producedQty: 1550, scrapQty: 78, status: 'running' },
  { id: 'job3', orderNumber: 'PRD-2026-003', producedQty: 0, scrapQty: 0, status: 'paused' },
];

// Fallback BOMs if localStorage is unpopulated
const FALLBACK_BOMS = [
  {
    id: 'bom1',
    productId: 'p1',
    name: 'تركيبة سبيكة الألومنيوم للأواني العميقة',
    version: 'v2.1',
    scrapPercentage: 2.5,
    items: [
      { materialId: 'ing-1', quantity: 1.2 }, // 1.2 kg raw material per unit
      { materialId: 'ing-2', quantity: 0.15 }, // 0.15 Liter chemical coat per unit
      { materialId: 'ing-3', quantity: 1.0 }  // 1 handle per unit
    ],
    routings: []
  },
  {
    id: 'bom2',
    productId: 'p2',
    name: 'تجميعة الغطاء الزجاجي المقسى بفتحة تنفيس',
    version: 'v1.0',
    scrapPercentage: 4,
    items: [
      { materialId: 'ing-4', quantity: 0.85 }, // 0.85 kg silicates glass raw per unit
      { materialId: 'ing-5', quantity: 1.0 }   // 1 steel rim outer ring
    ],
    routings: []
  }
];
...
export function CostManagement() {
  ...
  const [calcInputs, setCalcInputs] = useState({
    scrapCountFactor: 12,    // Scrap unit price (loss values)
    laborHoursNeeded: 48,   // Active runtime hours spent
    machineWearRate: 40,    // EGP/hour wear & tear
    machinePowerRate: 35,   // EGP/hour electricity
    machineRuntimeHours: 42,// Active hours run
    overheadPercentage: 15, // Indirect Factory Overheads %
    targetProfitMargin: 35, // Desired Profit Margin %
    targetSalePrice: 150    // User-estimated selling price
  });
  ...
  const loadLiveResources = async () => {
    ...
    try {
      ...
      if (hrResponse.ok) {
        ...
        setLiveEmployees(hrData);
        setIsHrLoaded(true);
        
        // Auto allocate first 3 employees as default workforce crew
        if (hrData.length > 0) {
          setAllocatedCrewIds(hrData.slice(0, 3).map((e: any) => e.id));
        }
      } else {
        // Mock fallback database if API fails
        const mockHr = [
          { id: 101, name: 'م. مروان الشافعي', job_title: 'كبير فنيي الصهر والسباكة', basic_salary: 12500, salary_type: 'monthly', department_name: 'صالة الصهر الحراري' },
          { id: 102, name: 'أ. جابر عبد الهادي', job_title: 'فني طلاء وتجليخ', basic_salary: 9200, salary_type: 'monthly', department_name: 'خط تكسية الأواني' },
          { id: 103, name: 'أ. محمود فوزي', job_title: 'مشغل مقص وضغط هيدروليكي', basic_salary: 350, salary_type: 'daily', department_name: 'قسم القطع والضغط' },
          { id: 104, name: 'أ. سيف اليماني', job_title: 'مفتش جودة الخامات الأولية', basic_salary: 8000, salary_type: 'monthly', department_name: 'صالة مراقبة الجودة' },
          { id: 105, name: 'أ. ناصر السعيد', job_title: 'مساعد تجهيز وتعبئة صناعية', basic_salary: 200, salary_type: 'daily', department_name: 'قسم التغليف والشحن' }
        ];
        setLiveEmployees(mockHr);
        setAllocatedCrewIds([101, 102, 103]);
        setIsHrLoaded(true);
      }
      ...
      if (ingResponse.ok) {
        ...
        setLiveIngredients(ingData);
        setIsInventoryLoaded(true);
      } else {
        // Fallback robust mock database
        const mockIng = [
          { id: 'ing-1', name: 'خام ألومنيوم مسبوك نقي درجة أولى', unit: 'كجم', cost: 110, current_stock: 4500, item_code: 'RAW-ALUM-01' },
          { id: 'ing-2', name: 'سائل تيفلون حراري أسود سويسري', unit: 'لتر', cost: 180, current_stock: 650, item_code: 'RAW-TEFL-X2' },
          { id: 'ing-3', name: 'مبقبض باكليت حراري مقاوم للحرارة', unit: 'قطع', cost: 15, current_stock: 8000, item_code: 'RAW-HNDL-P3' },
          { id: 'ing-4', name: 'رمل سيليكات معالج للغطاء الزجاجي', unit: 'كجم', cost: 45, current_stock: 2000, item_code: 'RAW-GLSS-SG' },
          { id: 'ing-5', name: 'إطار صلب كروم غير قابل للصدأ', unit: 'قطع', cost: 22, current_stock: 3500, item_code: 'RAW-RIM-SS' }
        ];
        setLiveIngredients(mockIng);
        setIsInventoryLoaded(true);
      }
      ...
      const prodData = savedProducts ? JSON.parse(savedProducts) : SAMPLE_PRODUCTS;
      const ordData = savedOrders ? JSON.parse(savedOrders) : SAMPLE_ORDERS;
      const wcData = savedWC ? JSON.parse(savedWC) : SAMPLE_WORK_CENTERS;
      const sfData = savedShopfloor ? JSON.parse(savedShopfloor) : SAMPLE_SHOPFLOOR;
      const bomData = savedBoms ? JSON.parse(savedBoms) : FALLBACK_BOMS;

      setProducts(prodData);
      setOrders(ordData);
      setWorkCenters(wcData);
      setShopfloorJobs(sfData);
      setBoms(bomData);

      // Default selected order
      if (ordData.length > 0 && !selectedOrderId) {
        setSelectedOrderId(ordData[0].id);
      }

      showToast('success', 'تم تجميع ومزامنة بيانات الموارد البشرية والمخازن الحية للـ ERP بنجاح.');
    } catch (e) {
      console.error('Error synchronizing multi-module costing data:', e);
      showToast('error', 'فشل في ربط المديولات تلقائياً. تم تشغيل المحاكي الاحتياطي.');
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <table className="w-full text-right text-xs">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {orders.filter(o => {
                ...
                const varianceValue = finalActCost - finalPlanCost;
                const variancePercent = (varianceValue / (finalPlanCost || 1)) * 100;
