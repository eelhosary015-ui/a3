...
const DEFAULT_ORDERS: any[] = [
  { id: '1', orderNumber: 'PRD-2026-001', productId: 'FIN-500', quantity: 15 },
  { id: '2', orderNumber: 'PRD-2026-002', productId: 'SFP-102', quantity: 200 }
];

const DEFAULT_CENTERS: any[] = [
  { id: '1', code: 'MC-101', name: 'غرفة القطع بالليزر (Laser cutting room)' },
  { id: '2', code: 'LN-001', name: 'خط التجميع الرئيسي (Main Assembly line)' }
];
...
const MOCK_JOBS: JobCard[] = [
  { 
    ...
    operation: 'القطع بالليزر للألواح الإنشائية', 
    workCenter: 'غرفة القطع بالليزر (Laser cutting room)', 
    workCenterId: '1',
    worker: 'المهندس أحمد محمود', 
    producedQty: 45, 
    scrapQty: 2, 
    startTime: '12-06-2026 08:00', 
    status: 'running',
    targetQty: 15,
    notes: 'سرعة القطع معايرة على خطوط سميكة'
  },
  { 
    id: 'JC-1002', 
    orderNumber: 'PRD-2026-001', 
    operation: 'التثقيب والتنعيم النهائي للمعادن', 
    workCenter: 'خط التجميع الرئيسي (Main Assembly line)', 
    workCenterId: '2',
    worker: 'الفني سيد سليمان', 
    producedQty: 12, 
    scrapQty: 0, 
    startTime: '12-06-2026 09:15', 
    status: 'paused',
    targetQty: 15,
    notes: 'توقف مؤقت لاستراحة الصيانة الدورية'
  },
];
...
export function ShopFloorControl() {
  ...
  useEffect(() => {
    const loadAll = async () => {
      const savedJobs = await databaseStorage.getItem<JobCard[]>('remo_production_shopfloor', MOCK_JOBS);
      const savedOrders = await databaseStorage.getItem<any[]>('remo_production_orders', DEFAULT_ORDERS);
      const savedCenters = await databaseStorage.getItem<any[]>('remo_production_workcenters', DEFAULT_CENTERS);
      ...
      setQualityChecks(savedChecks);
      setQualityAlerts(savedAlerts);
      setIsLoading(false);
    };
    loadAll();
  }, []);
