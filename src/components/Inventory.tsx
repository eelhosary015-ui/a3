...
interface Transaction {
  id: number; transaction_number: string; date: string; warehouse_id: number;
...
const TX_TYPE_LABELS: Record<string, string> = {
  receive: "استلام", issue: "صرف", transfer: "تحويل", opening: "افتتاحي",
  count: "جرد", adjustment: "تسوية", return: "مرتجع", damaged: "هالك",
};
const TX_EFFECT: Record<string, string> = {
  receive: "in", issue: "out", transfer: "transfer", opening: "in",
...
export function Inventory({ onBack, subView }: WarehouseModuleProps) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const fallbackItems: CountItemRow[] = ingredients.map((ing) => {
          ...
          if (targetWhId !== "all") {
            const row = inventoryItems.find(i => i.ingredient_id === ing.id && i.warehouse_id === Number(targetWhId));
            bookQty = row ? Number(row.quantity) : 0;
            const wh = warehouses.find(w => w.id === Number(targetWhId));
            if (wh) whName = wh.name;
          } else {
            bookQty = inventoryItems.filter(i => i.ingredient_id === ing.id).reduce((sum, r) => sum + Number(r.quantity), 0);
          }
          const cost = Number((ing as any).cost || (ing as any).cost_per_unit || (ing as any).cost_price || 0);
          return {
            product_id: ing.id,
            ingredient_id: ing.id,
            code: ing.code || `ITM-${ing.id}`,
            name: ing.name,
            unit: ing.unit || "قطعة",
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [whRes, ingRes, invRes, txRes, brRes, trfRes, movRes, supRes, dashRes, batRes, notifRes] = await Promise.all([
        ...
        api.get("/api/inventory-movements?limit=200"),
        api.get("/api/warehouse-suppliers"),
        api.get("/api/warehouse-dashboard"),
        api.get("/api/stock-batches"),
        api.get("/api/inventory-notifications?unread_only=true"),
      ]);
      if (whRes.ok) setWarehouses(await whRes.json().then(d => Array.isArray(d) ? d : d.data || []));
      if (ingRes.ok) setIngredients(await ingRes.json().then(d => Array.isArray(d) ? d : d.data || []));
      if (invRes.ok) setInventoryItems(await invRes.json().then(d => Array.isArray(d) ? d : d.data || []));
      if (txRes.ok) setTransactions(await txRes.json().then(d => Array.isArray(d) ? d : d.data || []));
      if (brRes.ok) setBranches(await brRes.json().then(d => Array.isArray(d) ? d : []));
      if (trfRes.ok) setTransfers(await trfRes.json().then(d => Array.isArray(d) ? d : d.data || []));
      if (movRes.ok) setMovements(await movRes.json().then(d => Array.isArray(d) ? d : d.data || []));
      if (supRes.ok) setSuppliers(await supRes.json().then(d => Array.isArray(d) ? d : d.data || []));
      if (dashRes.ok) setDashboard(await dashRes.json());
      if (batRes.ok) setBatches(await batRes.json().then(d => Array.isArray(d) ? d : d.data || []));
      if (notifRes.ok) setNotifications(await notifRes.json().then(d => Array.isArray(d) ? d : d.data || []));
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => exportToExcel(d.trend.map((t: any) => ({ التاريخ: t.date, الحركات: t.movements, المعاملات: t.transactions })), "movements-trend")}
              ...
              <tbody className="divide-y divide-slate-100">
                {movements.slice(0, 8).map(m => (
                  <tr key={m.id} className="hover:bg-slate-50">
                    ...
                    <td className="p-3"><span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-600">{m.ref_type}</span></td>
  ...
  const renderWarehouses = () => {
    const filtered = warehouses.filter(w => {
      ...
      const matchesModule = warehouseModuleFilter === "all" ||
        w.linked_module === warehouseModuleFilter ||
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-thin">
            {SYSTEM_MODULES.map(mod => {
              ...
              const count = mod.id === "all"
                ? warehouses.length
                : warehouses.filter(w => w.linked_module === mod.id || (w.linked_modules && String(w.linked_modules).includes(mod.id))).length;
              return (
                <button
                  key={mod.id}
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => exportToExcel(filtered.map((t: any) => ({ "رقم الحركة": t.transaction_number, التاريخ: t.date, المخزن: t.warehouse_name, النوع: TX_TYPE_LABELS[t.type] || t.type, السبب: t.reason, المرجع: t.reference, المستخدم: t.user, الحالة: t.status, "عدد الأصناف": t.items_count, القيمة: t.total_value })), "transactions")}
              className="flex items-center gap-2 px-4 py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-xl font-bold border border-emerald-200 transition-all">
              ...
              {data.map(tx => {
                ...
                return (
                  <tr key={tx.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-slate-600 text-xs">{tx.date}</td>
                    ...
                    <td className="p-4 text-center font-bold text-slate-700">{tx.items_count || 0}</td>
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map(tx => (
                  <tr key={tx.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-slate-600">{tx.reason || "—"}</td>
                    <td className="p-4 text-center font-bold">{tx.items_count || 0}</td>
                    <td className="p-4 font-bold text-purple-600">{fmtMoney(tx.total_value || 0)}</td>
                    <td className="p-4 text-xs text-slate-500">{tx.user}</td>
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4"><div className="flex items-center gap-3"><div className="w-10 h-10 bg-purple-50 rounded-xl flex items-center justify-center"><Calendar className="w-5 h-5 text-purple-600" /></div><div><p className="text-xs text-slate-500 font-bold">هالك هذا الشهر</p><p className="text-2xl font-black text-purple-600">{filtered.filter(t => (t.date || "").startsWith(new Date().toISOString().substring(0, 7))).length}</p></div></div></div>
        </div>
        <FilterBar />
        {filtered.length > 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => exportToExcel(filtered.map((m: any) => ({ التاريخ: m.created_at, الصنف: m.ingredient_name, المخزن: m.warehouse_name, "الحقل": m.field, "قبل": m.before_qty, "التغير": m.delta, "بعد": m.after_qty, "النوع": m.ref_type, "المرجع": m.ref_id, المستخدم: m.user, ملاحظات: m.notes })), "movements-log")}
            className="flex items-center gap-2 px-4 py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-xl font-bold border border-emerald-200 transition-all">
            <Download className="w-4 h-4" /> Excel
          </button>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {data.map(m => (
                <tr key={m.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-3"><span className={`px-2 py-0.5 rounded text-[10px] font-bold ${m.ref_type === "transaction" ? "bg-blue-50 text-blue-600" : m.ref_type === "transfer" ? "bg-cyan-50 text-cyan-600" : "bg-slate-100 text-slate-600"}`}>{m.ref_type}</span></td>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <table className="w-full text-right text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {(selectedTx.items || []).map((it, i) => (
                      <tr key={i}><td className="p-3 font-bold text-slate-800">{it.name}</td><td className="p-3 text-center">{it.quantity} {it.unit}</td><td className="p-3 text-center">{fmtMoney(it.price || 0)}</td><td className="p-3 text-center font-bold">{fmtMoney((it.quantity || 0) * (it.price || 0))}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
                ...
                <table className="w-full text-right text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {(selectedTransfer.items || []).map((it, i) => (
                      <tr key={i}><td className="p-3 font-bold text-slate-800">{it.name || ingredients.find(g => g.id === it.ingredient_id)?.name || "—"}</td><td className="p-3 text-center font-bold">{it.quantity}</td><td className="p-3 text-center text-slate-500">{it.unit}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
