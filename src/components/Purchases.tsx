...
export function Purchases({ onBack, initialTab, initialReport }: PurchasesProps) {
  ...
  const confirmApproveRequestToOrder = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data?.error || "فشل اعتماد طلب الشراء");
      setSelectedRequest(null);
      setRequestSupplierId("");
      setRequestDeliveryDate("");
      await Promise.all([fetchRequests(), fetchOrders(), fetchProcurementDashboard()]);
      setActiveTab("orders");
      alert(`تم اعتماد طلب الشراء وتحويله إلى أمر شراء ${data?.purchase_order?.order_number || `#${data?.purchase_order_id || data?.purchase_order?.id || ""}`}`);
  ...
  const renderReports = () => {
    const money = (value: any) => Number(value || 0).toLocaleString("ar-EG", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    const qty = (value: any) => Number(value || 0).toLocaleString("ar-EG", { maximumFractionDigits: 3 });
    ...
    return <div className="space-y-5">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5">
        ...
        </> : <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-slate-50"><tr>{(tableRows.length?Object.keys(tableRows[0]):(columnsByReport[reportSubTab]||["البيان","القيمة"])).map(h=><th key={h} className="p-3 text-right font-black text-slate-600 whitespace-nowrap">{h}</th>)}</tr></thead><tbody className="divide-y">{tableRows.length?tableRows.map((row,i)=><tr key={i} className="hover:bg-slate-50">{Object.keys(row).map(k=><td key={k} className="p-3 font-bold text-slate-700 whitespace-nowrap">{typeof row[k] === "number" ? money(row[k]) : String(row[k] ?? "-")}</td>)}</tr>):<tr><td colSpan={(columnsByReport[reportSubTab]||["البيان"]).length} className="p-12 text-center text-slate-400 font-bold">لا توجد بيانات مطابقة للفلاتر الحالية</td></tr>}</tbody></table></div>}
      </div>
    </div>;
  };
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 print:hidden">
          {[
            ...
            <div key={String(label)} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
              ...
              <div className="text-lg font-black text-slate-900">{Number(value || 0).toLocaleString()} <span className="text-[10px] text-slate-400">{unit}</span></div>
            </div>
          ))}
        </div>
      )}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {req.purchase_order_id ? (
                        ...
                        <>
                          ...
                          >
                            <CheckCircle2 className="w-4 h-4" /> اعتماد كأمر شراء
                          </button>
                          <button
            ...
            <table className="w-full text-right min-w-max">
              <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                <tr>
                  ...
                  <th className="p-4 font-bold text-slate-600">رقم الفاتورة الداخلي</th>
                  <th className="p-4 font-bold text-slate-600">رقم فاتورة المورد</th>
                  <th className="p-4 font-bold text-slate-600">
                    تاريخ الفاتورة
                  </th>
                  <th className="p-4 font-bold text-slate-600">المورد</th>
              ...
              <tbody>
                {filteredInvoices.map((inv, idx) => (
                  ...
                  >
                    ...
                    <td className="p-4 font-mono text-slate-600 text-left" dir="ltr">
                ...
                <table className="w-full text-right min-w-max text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {purchaseItems.map((item:any, idx:number) => (
                      <tr key={`${item.ingredient_id}-${idx}`} className="hover:bg-slate-50">
                        ...
                        <td className="p-3 text-center text-slate-700">{Number(item.quantity||0).toLocaleString()}</td>
                        <td className="p-3 text-center">{Number(item.unit_price||0).toLocaleString()}</td>
                        <td className="p-3 text-emerald-600 font-bold">{(Number(item.quantity||0)*Number(item.unit_price||0)).toLocaleString()} ج.م</td>
                      </tr>
                    ))}
      ...
      {selectedQuotation && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl p-6">
            ...
            <div className="border rounded-xl overflow-hidden mb-5"><table className="w-full text-right text-sm"><thead className="bg-slate-100"><tr><th className="p-3">الصنف</th><th className="p-3">الوحدة</th><th className="p-3">الكمية</th><th className="p-3">سعر الوحدة</th><th className="p-3">الإجمالي</th></tr></thead><tbody className="divide-y">{(Array.isArray(selectedQuotation.items)?selectedQuotation.items:[]).map((it:any,i:number)=><tr key={i}><td className="p-3 font-bold">{it.name||'-'}</td><td className="p-3">{it.unit||'-'}</td><td className="p-3">{Number(it.quantity||0).toLocaleString()}</td><td className="p-3">{Number(it.unit_price||0).toLocaleString()}</td><td className="p-3 font-bold">{(Number(it.quantity||0)*Number(it.unit_price||0)).toLocaleString()}</td></tr>)}</tbody></table></div>
